...
func TestFiles_HandleGetFile_UserExistsFileNotFound(t *testing.T) {
	...
	bytes, _ := json.Marshal(payload)
	request, _ := http.NewRequest("PUT", "files/test111.txt", strings.NewReader(string(bytes)))
	mapper := map[string]string{
...
func TestFiles_HandleGetFile_UserExistsFileFound(t *testing.T) {
	...
	bytes, _ := json.Marshal(payload)
	request, _ := http.NewRequest("PUT", "files/test4.txt", strings.NewReader(string(bytes)))
	mapper := map[string]string{
		"fileName": "test4.txt",
	}
	request = mux.SetURLVars(request, mapper)
...
func TestFiles_HandleGetFile_NotFound(t *testing.T) {
